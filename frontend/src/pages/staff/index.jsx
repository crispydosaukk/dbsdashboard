...
import {
  Edit2, Save, Loader2, User, UserCheck, Camera, Briefcase, Shield, Calendar, Eye, EyeOff, Clock, XCircle, Plus,
  Users, Search, X, Building2, Phone, Mail, ShieldCheck, ShieldOff, ChevronRight, Printer, FileText, Download, Bell, Store, PoundSterling, Trash2, CheckCircle, AlertCircle, ArrowRight, Upload, XIcon, Send
} from "lucide-react";
  ...
  const handleEmailReport = async () => {
    ...
    try {
      ...
      if (isIndividualView || isFilterEmployeeView) {
        ...
        let filteredRecs = isIndividualView 
        ...
        grouped.forEach(sg => {
          ...
          tableRows += `<tr style="background-color:#f8fafc;border-bottom:2px solid #cbd5e1;">
            <td style="padding:10px 12px;font-size:12px;font-weight:700;color:#475569;" colspan="2">TOTAL</td>
            <td style="padding:10px 12px;font-size:13px;font-weight:800;color:#0f172a;text-align:right;" colspan="2">${tHrs}h ${tMins}m ${rate > 0 ? `&nbsp;|&nbsp; <span style="color:#D0B079;">£${totalPay.toFixed(2)}</span>` : ''}</td>
          </tr>`;
        });
        
        if (grouped.length === 0) {
          tableRows = '<tr><td colspan="4" style="text-align:center;padding:20px;color:#6b7280;">No records found</td></tr>';
        } else {
          const grandTotalMins = grouped.reduce((sum, g) => sum + g.total_minutes, 0);
          const grandHrs = Math.floor(grandTotalMins / 60);
          const grandMins = grandTotalMins % 60;
          tableRows += `<tr style="background-color:#0b1a3d;border-top:3px solid #1e3a5f;">
            <td style="padding:16px 12px;font-size:14px;font-weight:800;color:white;text-transform:uppercase;letter-spacing:1px;" colspan="2">GRAND TOTAL OUTPUT</td>
            <td style="padding:16px 12px;font-size:16px;font-weight:900;color:#D0B079;text-align:right;" colspan="2">${grandHrs}h ${grandMins}m</td>
          </tr>`;
        }
      }
...
<body>
  ...
  }, [attendanceData?.records, typeof staff !== 'undefined' ? staff : [], typeof reportEmployeeFilter !== 'undefined' ? reportEmployeeFilter : null]);
